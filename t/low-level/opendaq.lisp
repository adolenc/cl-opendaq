...
(test opendaq-config-provider
  (opendaq.low-level:with-daq-objects (config-provider)
    ...
    (is (not (cffi:null-pointer-p config-provider)) "opendaq/opendaq ConfigProvider returned a null object")))
...
(test opendaq-instance-and-builder
  (opendaq.low-level:with-daq-objects (builder instance)
    ...
    (is (not (cffi:null-pointer-p builder)) "opendaq/opendaq InstanceBuilder returned a null object")
    (setf instance (%make-test-instance))
    (is (not (cffi:null-pointer-p instance)) "opendaq/opendaq Instance returned a null object")))
