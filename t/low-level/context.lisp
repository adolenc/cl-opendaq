...
(test opendaq-context
  (opendaq.low-level:with-daq-objects (context out-logger out-type-manager out-options out-discovery-servers)
    ...
    (is (not (cffi:null-pointer-p out-logger)) "opendaq/context logger lookup returned null")
    (is (not (cffi:null-pointer-p out-type-manager)) "opendaq/context type manager lookup returned null")
    (is (not (cffi:null-pointer-p out-options)) "opendaq/context options lookup returned null")
    (is (not (cffi:null-pointer-p out-discovery-servers)) "opendaq/context discovery servers lookup returned null")))
