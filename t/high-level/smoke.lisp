...
(test high-level-component-type-detection
  (let* ((instance (make-instance 'daq:instance))
    ...
    (is (daq:supports-interface-p channel 'daq:folder) "A channel should support IFolder (a channel is a function block).")
    (is (not (daq:supports-interface-p signal 'daq:folder)) "A signal should not support IFolder (the failure path must not crash).")))
