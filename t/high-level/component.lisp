...
(test high-level-component-hierarchy
  (let* ((sinks (make-instance 'daq:object-list))
    ...
    (let* ((logger (make-instance 'daq:logger :sinks sinks :level :daq-log-level-debug))
      ...
      (is (eq 'daq:component (daq:component-type child))
          "component-type should report a plain component as COMPONENT.")
      (is (daq:supports-interface-p child 'daq:component)
          "supports-interface-p should confirm a component implements IComponent.")
      (is (not (daq:supports-interface-p child 'daq:channel))
          "supports-interface-p should return NIL (not crash) for an unsupported interface."))))
