# openDAQ Common Lisp bindings

Common Lisp bindings for interacting with [openDAQ](https://opendaq.com/) devices. Tested with SBCL, and comes bundled with binaries for x64 Linux, x64 Windows, and ARM macOS.

## Quickstart

Install the library as a [Quicklisp](https://www.quicklisp.org/beta/) local project by cloning it into `~/quicklisp/local-projects/opendaq/`.

Then, run the following in your REPL:
```lisp
(ql:quickload :opendaq)

(defparameter *instance* (make-instance 'daq:instance))

(format t "Available devices: ~%~{ - ~A~%~}~%"
  (mapcar #'daq:connection-string (daq:available-devices *instance*)))

(let* ((device (daq:add-device *instance* "daqref://device0"))
       (channel (daq:as (daq:find-component *instance* "Dev/RefDev0/IO/AI/RefCh0") 'daq:channel))
       (signal (first (daq:signals channel)))
       (reader (make-instance 'daq:stream-reader :signal signal)))
  (setf (daq:property-value device "GlobalSampleRate") 100
        (daq:property-value channel "Frequency") 0.5)
  (daq:read reader 100 :timeout-ms 2000))
```

This should give you an output of 100 samples from the simulator device set to run at 100Hz, a sine wave at 0.5Hz.

If that fails, run a healthcheck to verify that the library can find and load the native openDAQ libraries correctly:
```lisp
(daq:healthcheck)
```

## Some details

The library is composed of 2 packages: `:opendaq` (nicknamed `:daq`) and `:opendaq.low-level`:
 - `:opendaq.low-level` contains low-level bindings that closely mirror the C API by utilizing CFFI. These are autogenerated from the C header files (in `bindings/c/`) and are not intended for direct use by end-users.
 - `:opendaq` contains a high-level API that wraps the low-level bindings in a more idiomatic and user-friendly way. This is the intended interface for end-users.

The bindings depend only on plain `cffi`. A few C functions that take a struct by value (the `daqIntfID` GUID) are intentionally skipped during generation, since marshalling by-value structs would require `cffi-libffi`, which compiles a C shim at load time and so fails on hosts without a C compiler (e.g. a stock Windows install). The one such function we hand-write (`borrowInterface`, backing `supports-interface-p`/`component-type`) sidesteps this by passing the 16 opaque GUID bytes per the platform ABI — two 64-bit words on Linux/macOS, a pointer on Windows.

The high-level API uses CLOS to attempt to mirror openDAQ classes and their hierarchy, and uses generics/methods on them for function calls. It drops the get/set prefixes from the function names, and instead uses `setf` wrappers for setters.

A method is normally named by its bare stem (`signals`, `lock`): one generic, shared across every class that has it. When the same call takes different arities across classes (CLOS requires congruent lambda lists), it is either folded into one generic with trailing `&optional` parameters or given a receiver-prefixed name (`device-private-lock`). Note that a leading word isn't always such a prefix — it may be openDAQ's own (`getDeviceRevision` → `device-revision`) or a static per-interface method with no receiver to dispatch on (`getInterfaceId` → `<class>-interface-id`).

The hierarchy is parsed automatically from the C++ headers via the `DECLARE_OPENDAQ_INTERFACE(IChild, IParent)` macros. The root of the hierarchy is `managed-object`, which owns a raw pointer and registers a `trivial-garbage` finalizer to call `release-ref` when the wrapper is collected, so you never have to release openDAQ objects manually.

### Boxing / unboxing

The high-level API insulates you from openDAQ's C types. Arguments accept plain Lisp values (boxing, on input) and return values are plain Lisp types (unboxing, on output); the two directions are inverses, so one table covers both:

| Lisp value | openDAQ type |
|------------|--------------|
| `string`   | `daqString` |
| `integer`  | `daqInteger` |
| `float`    | `daqFloat` |
| `ratio`    | `daqRatio` |
| `t` / `nil` | `daqBool` |
| `(complex re im)` | `complexNumber` |
| any `daq` object | the openDAQ object itself (its raw pointer is extracted on input, the result re-wrapped on output) |

`daqNumber` is output-only: it unboxes to a `float`, but no Lisp type boxes to it specifically (a `float` always boxes to `daqFloat`).

**Manual conversion.** A few functions let you cross the boundary explicitly when needed:

- `(as obj 'device-info)` - cast a wrapper to a more specific type (only valid when `obj` really is that type)
- `(supports-interface-p obj 'channel)` - test whether `obj` implements an interface
- `(component-type obj)` - the most-derived component interface `obj` implements (`channel`, `signal`, `device`, …) as a class symbol, or `nil`
- `(as-list-of obj-list 'device-info)` - object-list to a plain Lisp list, with each element cast (or unboxed, for a primitive type)
- `(as-hashtable-of dict 'string 'device-info)` - dict to hash-table
- `(value-of obj)` / `(value-of obj 'daq-string-object)` - unbox a boxed primitive (number, string, bool, ratio, …) to its Lisp value; the type can be omitted for a typed wrapper but is needed for a generic base-object
- `(raw-pointer obj)` - get the underlying C pointer from any wrapper
- `(release obj)` - manually drop the reference (normally handled by GC finalizers)
- `(domain-tick->timestamp source tick)` - convert a domain tick to a `local-time` timestamp (`source` is a signal, data descriptor, or multi reader; `(domain-time-converter source)` returns a reusable per-tick closure)

### Construction

Everything is built with `make-instance`, e.g. `(make-instance 'daq:stream-reader :signal signal)`.

Some things are not distinct openDAQ interfaces but *variants* of one type that openDAQ builds through factory functions - for example a property is always an int/string/selection/… property, never a bare `Property`. These (and a type's alternate constructors) are exposed as `base/variant` subclasses you build the same way, with keyword arguments:

```lisp
(make-instance 'daq:property/int :name "Frequency" :default-value 10 :visible t)
(make-instance 'daq:property/selection :name "Mode" :selection-values modes :default-value 0 :visible t)
(make-instance 'daq:data-rule/linear :delta 2 :start 0)
(make-instance 'daq:logger-sink/std-err)
(make-instance 'daq:stream-reader/from-port :port port)   ; an alternate stream-reader constructor
```

Each such class is a subclass of the type it builds (`(typep (make-instance 'daq:property/int …) 'daq:property)` is true), so the result works anywhere that type is expected. This covers property builders, data/dimension rules, search filters, scalings, logger sinks, and similar factory-built types. (`apropos` for `/` lists them.)

### Events

openDAQ events (core/structure events, property change notifications, …) are subscribed to by passing a Lisp function straight to `add-handler`; it is called with the sender and event args (already wrapped) each time the event fires, and reference counting is handled for you:

```lisp
(daq:add-handler (daq:on-core-event (daq:context instance))
                 (lambda (sender args)
                   (let ((event (daq:as args 'daq:core-event-args)))
                     (format t "~A~%" (daq:event-name event)))))
```

`add-handler` returns the created `event-handler`, which can be passed to `remove-handler` to unsubscribe. See `examples/core-events.lisp`.

## Development

The attached makefile is used for simplification of development. It handles cloning the pinned openDAQ source, building the native libraries, copying them into `bin/linux-x64/`, and regenerating the generated Lisp bindings. It also has targets for starting a REPL with the system loaded and running the test suite.

### Makefile

```bash
make bindings # clone pinned openDAQ source, build native libs, copy them into bin/linux-x64/, and call `make regenerate-bindings`
make regenerate-bindings # regenerate generated/low-level-bindings.lisp and generated/high-level-bindings.lisp from the C headers in the cloned openDAQ source
make repl     # start SBCL with the opendaq system loaded
make test     # run the FiveAM test suite
make clean    # remove bin/ and tmp/
```

Override `OPENDAQ_REF` in the Makefile if you want a different upstream version. Override `OPENDAQ_RUNTIME_TRIPLE` if you want `make bindings`, `make repl`, and `make test` to use a different `bin/<platform>/` directory name.

### Folder structure

- `*.lisp/.asd` - handwritten lisp files for the run-time API
- `generated/` - autogenerated low-level and high-level bindings
- `tools/` - scripts used to generate the bindings in `generated/` (note that `tools/parse_bindings.py` is a generic python script that can be used to generate bindings for any language)
- `t/` - FiveAM test files, mostly ported from the c bindings test suite
- `bin/` - bundled native openDAQ libraries loaded at runtime, split by platform subdirectory
- `tmp/` - temporary clone/build workspace used by `make bindings`
