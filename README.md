...
The bindings depend only on plain `cffi`. The one C type passed by value, the `daqIntfID` GUID, would normally need `cffi-libffi` (which compiles a C shim at load time and so fails on hosts without a C compiler, e.g. a stock Windows install); the generator instead passes its 16 opaque bytes per the platform ABI — two 64-bit words on Linux/macOS, a pointer on Windows — so no shim is needed.
